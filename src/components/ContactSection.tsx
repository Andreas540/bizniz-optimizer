...
function validate(f: Fields): Errors {
  const e: Errors = {};
  if (!f.name.trim()) e.name = "Please try this field again";
  if (!f.email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(f.email))
    e.email = "Please try this field again";
  if (!f.phone.trim() || !/^\d{4,}$/.test(f.phone.replace(/\s/g, "")))
    e.phone = "Please try this field again";
  return e;
}
